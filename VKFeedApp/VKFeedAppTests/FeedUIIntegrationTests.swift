...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadFeedActions_requestFeedFromLoader() {
        ...
        XCTAssertEqual(loader.loadFeedCallCount, 1, "Expected no request until previous is not completed")
        
        loader.completeFeedLoading(at: 0)
        sut.simulateUserInitiatedReload()
        XCTAssertEqual(loader.loadFeedCallCount, 2, "Expected another loading request once user initiates a reload after previous request completed")
        
        loader.completeFeedLoading(at: 1)
        sut.simulateUserInitiatedReload()
        XCTAssertEqual(loader.loadFeedCallCount, 3, "Expected yet another loading request once user initiates another reload after previous request completed")
    }
    ...
    func test_feedViewIsVisible_loadsImageFromURL() {
        ...
        XCTAssertTrue(loader.loadedImageUrls.isEmpty, "Expected not to load image urls until view is not visible")
    ...
    func test_feedViewIsHidden_cancelsLoadImageFromURL() {
        ...
        sut.loadViewIfNeeded()
        loader.completeFeedLoading(feed: [image0, image1], at: 0)
        XCTAssertTrue(loader.cancelledImageURLs.isEmpty, "Expected not to cancel load image urls until view becomes hidden")
        
        sut.simulateFeedImageViewHidden(at: 0)
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url], "Expected to cancel load url for first image")
    
        sut.simulateFeedImageViewHidden(at: 1)
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url, image1.url], "Expected to cancel load url for second image while first remains cancelled from the previous step")
    }
    ...
    func test_feedImageView_doesNotLoadImageAgainUntilPreviousRequestCompletes() {
        ...
        let (loader, sut) = makeSUT()
        sut.loadViewIfNeeded()
        loader.completeFeedLoading(feed: [image], at: 0)
        
        sut.simulateFeedImageViewNearVisible(at: 0)
        XCTAssertEqual(loader.loadedImageUrls, [image.url], "Expected the image to load when near visible")
